...
class IsAdminOrScolariteReadOnly(BasePermission):
    """
    Autorisation : Admin (tout) ou Scolarité (lecture seule).
    """
    def has_permission(self, request, view):
        ...
        if request.user.role == 'ADMIN':
            return True
        if request.user.role == 'SCOLARITE' and request.method in ['GET', 'HEAD', 'OPTIONS']:
            return True
        return False
